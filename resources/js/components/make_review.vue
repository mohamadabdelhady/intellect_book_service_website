<template>
  <div>
    <div v-if="!my_review.length" class="review p-2">
      ...
      <span class="mt-2 stars" style="padding-left: 60px">
        <a
          ...
          class=""
          id="star_1"
          @mouseover="fill_star(1)"
          @mouseleave="empty_star()"
          v-on:click.prevent="set_rating(1)"
        ...
        <a
          ...
          class=""
          id="star_2"
          @mouseover="fill_star(2)"
          @mouseleave="empty_star()"
          v-on:click.prevent="set_rating(2)"
          ><i class="fa fa-star fa-3x"></i
        ></a>
        <a
          href="#"
          class=""
          id="star_3"
          @mouseover="fill_star(3)"
          @mouseleave="empty_star()"
          v-on:click.prevent="set_rating(3)"
          ><i class="fa fa-star fa-3x"></i
        ></a>
        <a
          href="#"
          class=""
          id="star_4"
          @mouseover="fill_star(4)"
          @mouseleave="empty_star()"
          v-on:click.prevent="set_rating(4)"
          ><i class="fa fa-star fa-3x"></i
        ></a>
        <a
          href="#"
          class=""
          id="star_5"
          @mouseover="fill_star(5)"
          @mouseleave="empty_star()"
          v-on:click.prevent="set_rating(5)"
          ><i class="fa fa-star fa-3x"></i
        ></a>
        <span class="ml-2 rate">{{ my_rating }}/5</span>
        <span v-if="my_rating != ''" class="ml-2 rate"
          ><a href="#" v-on:click.prevent="clear_rating">Clear rating</a></span
        >
      </span>
      <div class="mt-3">
        <textarea
          ...
          class="comment_in"
          v-model="user_review"
          @keyup.enter="add_review"
        ></textarea>
        <button
          v-if="edit_review == false"
          class="btn"
          style="height: 40px"
          :disabled="user_review == ''"
          v-on:click="add_review"
        >
          Post
        </button>
        <div v-else>
        ...
        <p id="error_post" style="color: firebrick; display: none">
          You have to pick a rating to post a review.
        </p>
      </div>
    </div>
    <div v-else class="my_review p-2" v-for="(review, index) in my_review">
      ...
      <a href="#"
        ><span class="pl-1">{{ review.name }}</span></a
      >
